#!/usr/bin/env python3
"""Batch Qwen-Image-Edit-2511 inference for the BDD100K insertion schedule.

Source images are read relative to the BDD100K data directory using the
``image`` path from each schedule task. The schedule has no separate mask
path, so each source image is supplied directly to the edit pipeline.
"""
...
DEFAULT_DATA_ROOT = Path("/home/qinma/yelo/datasets/bdd100k_val1000_common")
DEFAULT_MODEL_PATH = Path("/home/qinma/yelo/models/Qwen-Image-Edit-2511")
DEFAULT_OUTPUT_ROOT = Path("/home/qinma/yelo/outputs/Qwen-Image-Edit-2511_BDD100K")
...
def source_for(root: Path, task: dict[str, Any]) -> Path:
    """Return the source path from the schedule, with a legacy-safe fallback."""
    image_path = task.get("image")
    if image_path:
        return root / str(image_path)
    return root / "img" / f"{task['frame_stem']}.jpg"
...
def prompt_for(task: dict[str, Any]) -> str:
    ...
    return (
        "Preserve all existing objects and keep the rest of the image unchanged. "
        f"Insert additional {objects} into the road scene. "
        "Match the original driving-camera perspective, object scale, lighting, "
