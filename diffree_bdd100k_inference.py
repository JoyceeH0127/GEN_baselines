#!/usr/bin/env python3
"""Batch Diffree inference for a BDD100K insertion schedule.
...
DEFAULT_OUTPUT_ROOT = Path("/home/qinma/yelo/outputs/Diffree_BDD100K")
...
def source_for(root: Path, task: dict[str, Any]) -> Path:
    image_value = task.get("image")
    image = (
        Path(image_value)
        if image_value
        else Path("img") / f"{task['frame_stem']}.jpg"
    )
    return image if image.is_absolute() else root / image
...
def prompt_for_object(object_name: str) -> str:
    return (
        ...
        f"Insert 1 additional {object_name} into the road scene. "
