...
All positive class counts for a frame are expressed in one target prompt and
generated with one Add-it call. For example, five requested cars uses
"5 cars" in the target prompt and "cars" as the subject token.
...
CLASS_NAMES = (
    ...
    ("awning-tricycle", "awning-tricycle", "awning-tricycles"),
    ("bus", "bus", "buses"),
...
def arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--source-prompt",
        default=(
            "A realistic low-resolution aerial surveillance photograph of an "
            "urban road scene, preserving the original camera view, lighting, "
...
def insertion_for(task: dict[str, Any]) -> dict[str, Any]:
    """Combine all requested counts into one Add-it insertion specification."""
    counts = task["class_names"]
    requested: list[dict[str, Any]] = []
    for json_name, singular, plural in CLASS_NAMES:
    ...
    phrases = [item["phrase"] for item in requested]
    object_phrase = (
        phrases[0]
        if len(phrases) == 1
        else ", ".join(phrases[:-1]) + f" and {phrases[-1]}"
    )
    return {
        "requested": requested,
        "object_phrase": object_phrase,
        "subject_token": " ".join(item["noun"] for item in requested),
    }


def target_prompt(source_prompt: str, object_phrase: str) -> str:
    return f"{source_prompt}, with {object_phrase} added to the scene."
...
def main() -> int:
    ...
    if args.dry_run:
        for index, task in indexed_tasks:
            ...
            insertion = insertion_for(task)
            target = target_prompt(source_prompt, insertion["object_phrase"])
            print(
                json.dumps(
                    {
                        "index": index,
    ...
    for position, (index, task) in enumerate(indexed_tasks, start=1):
        ...
        try:
            ...
            prompt_target = target_prompt(
                source_prompt, insertion["object_phrase"]
            )
            print(
                f"[{position}/{len(indexed_tasks)}] {source_path} "
                f"object={insertion['object_phrase']!r} "
                f"subject_token={insertion['subject_token']!r}"
            )
            print(f"  target_prompt: {prompt_target}")
            current = runner.insert_once(
                current,
                source_prompt,
                prompt_target,
                insertion["subject_token"],
                task_seed,
                task_seed,
                args,
            )
