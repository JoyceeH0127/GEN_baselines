#!/usr/bin/env python3
"""Batch Qwen-Image-Edit-2511 inference aligned with the FLUX.2 UAVDT run.

This script consumes the same schedule JSON, resolves the same UAVDT paths,
builds the same object-count prompt, writes the same output naming layout, and
records a compatible JSONL manifest. The schedule has no separate mask path;
input images are therefore assumed to already contain the masked regions.
"""
...
DEFAULT_OUTPUT_ROOT = Path("/home/qinma/yelo/outputs/Qwen-Image-Edit-2511_UAVDT")
...
def resolve_source(path: str, data_root: Path) -> Path:
    ...
    if OLD_ROOT_MARKER not in normalized:
        raise ValueError(f"Cannot replace UAVDT root in path: {path}")
    relative = normalized.split(OLD_ROOT_MARKER, 1)[1]
